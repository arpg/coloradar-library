...
void H5Run::setData(
    ...
    if (!cascadeCubeTimestamps.empty() && !cascadeDatacubes.empty() && cascadeCubeTimestamps.size() != cascadeDatacubes.size())
        throw std::invalid_argument("H5Run::setData() Length Mismatch: 'cascadeCubeTimestamps' (" + std::to_string(cascadeCubeTimestamps.size()) + ") vs 'cascadeDatacubes' (" + std::to_string(cascadeDatacubes.size()) + ").");
