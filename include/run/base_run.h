...
public:
    ...
    // abstract methods
    virtual std::vector<int16_t> getCascadeDatacube(const int cubeIdx) const = 0;
    virtual std::vector<float> getCascadeHeatmap(const int hmIdx) const = 0;
    virtual pcl::PointCloud<RadarPoint>::Ptr getCascadePointcloud(const int cloudIdx, const double intensityThreshold = 0.0) = 0;
